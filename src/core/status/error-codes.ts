...
export const ERROR_CODES: Record<ErrorCode, ErrorDoc> = {
  // ---------- Project/System/Zone validation ----------
  "PROJ.MISSING_FIELDS": {
    severity: "error",
  ...
  "SYS.INVALID_CHARS": {
    ...
    message: () => "System names may not include any of: * ? : \\ / [ ]",
  ...
  "SYS.TANK_CAPACITY": {
    ...
    message: (p) =>
      `Required water tank capacity (${p?.reqGal ?? "?"} gal) exceeds the maximum available for "${p?.certPretty ?? "selected cert"}" (${p?.maxGal ?? "?"} gal). Supply a tank rated for at least ${p?.reqGal ?? "?"} gal.`,
    resolution:
  ...
  "ZONE.INVALID_CHARS": {
    ...
    message: () => "Zone names may not include any of: * ? : \\ / [ ]",
  ...
  "ENC.FMDC_VOLUME_LIMIT": {
    ...
    appearsWhen: "On Validate",
    meaning: "The enclosure volume exceeds the FM DC limit.",
    message: () =>
      "Volume for FM Data Centers may not exceed 31,350 ft³ (2,912.5 m³).",
    resolution: "Split the space or change the design method.",
  },
  "ENC.FM_VOLUME_LIMIT": {
    severity: "error",
    ...
    resolution: "Divide the space or adjust the design method.",
  },
  "ENC.CUSTOM_EMITTERS": {
    severity: "warn",
    title: "Emitter count modified from computed minimum",
    appearsWhen: "On Calculate",
    meaning:
      "The user override for emitters is modified from the computed minimum.",
    message: (p) =>
      `Custom emitters modified from the computed minimum for "${p?.name || "Enclosure"}": ${p?.final ?? "?"} (calc: ${p?.calc ?? "?"}).`,
  ...
  "ENC.NFPA_MAX_DISCHARGE": {
    ...
    message: (p) =>
      `Estimated discharge time ${p?.t_est ?? "?"} min exceeds 3.0 min for "${p?.name || "Enclosure"}". Increase emitters or nitrogen flow.`,
    resolution: "Increase emitters or select a higher-flow nozzle.",
  },
  "ENC.FMDC_MIN_DISCHARGE": {
    severity: "error",
    title: "FM DC minimum discharge time not met",
    appearsWhen: "On Calculate",
    meaning: "Calculated discharge time is below 3.5 minutes (FM DC).",
    message: (p) =>
      `FM Data Centers minimum discharge time is 3.5 minutes (actual: ${p?.t_actual ?? "?"}).`,
    resolution: "Add cylinders or reduce flow until t ≥ 3.5 minutes.",
  },
  "ENC.N2_NOT_MET": {
    severity: "warn",
    ...
    appearsWhen: "On Calculate",
    meaning:
      "Delivered nitrogen is below required or final O2 exceeds the threshold.",
    message: (p) =>
      `Nitrogen requirement not met for "${p?.name || "Enclosure"}". Delivered ≈ ${p?.delivered ?? "?"} SCF, required ≈ ${p?.required ?? "?"} SCF. Discharge time is invalid; reduce nozzle size or increase cylinders.`,
    resolution: "Reduce nozzle size or increase the cylinder count.",
  },
  "ENC.CYL_LIMIT": {
    severity: "error",
    ...
    resolution: "Split the space, switch to engineered, or reduce volume.",
  },
  "ENC.O2_HIGH": {
    severity: "error",
    title: "Final oxygen too high",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "Final oxygen exceeds 14.1%.",
    message: () =>
  ...
  "ENC.TIME_CONSTRAINT": {
    severity: "error",
    title: "No feasible emitters/pressure",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "No valid integer emitter count meets the time window.",
    message: (p) =>
  ...
  "ENC.EMITTER_STYLE": {
    severity: "error",
    title: "Emitter style incompatible",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "The selected style is not allowed for the method/nozzle.",
    message: () =>
      "Selected emitter style is incompatible with the design method/nozzle.",
  ...
  "ENC.HEIGHT_LIMIT": {
    ...
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning:
      "Ceiling height exceeds the FM limit for the selected emitter size.",
    message: () =>
      "Ceiling height exceeds maximum allowable for the selected emitter size per FM approval.",
    resolution: "Lower the height, change emitter size, or redesign layout.",
  },
  "ENC.FM_SPACING": {
    severity: "error",
    title: "FM spacing violated",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "Room dimensions violate FM emitter spacing rules.",
    message: () =>
      "Room dimensions violate FM requirements for emitter spacing.",
    resolution: "Increase emitters, change size, or divide the room.",
  },
  "ENC.O2_LOW_MOD": {
    severity: "warn",
    title: "Low O2 (moderate)",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "Final oxygen is between 10% and 12%.",
    message: () =>
  ...
  "ENC.O2_LOW_SUB": {
    severity: "warn",
    title: "Low O2 (substantial)",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "Final oxygen is between 8% and 10%.",
    message: () =>
  ...
  "ENC.O2_VERY_LOW": {
    ...
    title: "Very low O2",
    appearsWhen: "On Calculate (Pre-Eng)",
    meaning: "Final oxygen is below 8%.",
    message: () =>
